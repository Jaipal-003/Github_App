...
let /**{HTMLElement} */ [$lastActiveTabBtn] = $tabBtns;
let /**{HTMLElement} */ [$lastActiveTabPanel] = $tabPanels;
...
addEventOnElement($tabBtns, "click", function () {
  ...
  const /**{HTMLElement} */ $currentTabPanel = document.querySelector(
    `#${this.getAttribute("aria-controls")}`/**{see the changes} */
  );
 ...
 * Profile
 */
const /**{HTMLElement} */ $profileCard = document.querySelector("[data-profile-card]");
const /**{HTMLElement} */ $repoPanel = document.querySelector("[data-repo-panel]");
const /**{HTMLElement} */ $error = document.querySelector("[data-error]");
  ...
  fetchData(`${repoUrl}?sort=created&per_page=12`,function(data){
    ...
    if(repositories.length){
      for(const repo of repositories){
        const{
          name,html_url,description,private:isPrivate,
          language,stargazers_count : stars_count,forks_count
        }=repo;
